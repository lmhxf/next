export const goods = [
  {
    type: 'recommend',
    data: [
      {
        id: '01', prod: '68', imgFile: 'recommend/01', name: '新款1315超高压航空合金电击棍', price: '340',
        detail: {
          banner: ['recommend/01/banner1', 'recommend/01/banner2'],
          goodId: '1315', // 型号
          goodImg: 'recommend/01/1315',
          buyPrice:'510',   //市场价
          cash: 9999,   // 库存
          detailImg: ['recommend/01/1315-1', 'recommend/01/1315-2', 'recommend/01/1315-3', 'recommend/01/1315-4', 'recommend/01/1315-5']
        }
      },
      {
        id: '02', prod: '91', imgFile: 'recommend/02', name: '新款金刚狼牙007高压电棍', price: '300',
        detail: {
          banner: ['recommend/02/banner1', 'recommend/02/banner2', 'recommend/02/banner3', 'recommend/02/banner4', 'recommend/02/banner5'],
          goodId: '07',
          goodImg: 'recommend/02/007',
          buyPrice: '360',
          cash: 9999,
          detailImg: ['recommend/02/007-01', 'recommend/02/007-02', 'recommend/02/007-03']
        }
      },
      {
        id: '03', prod: '90', imgFile: 'recommend/03', name: '8810可调强光高压电击棍', price: '240',
        detail: {
          banner: ['recommend/03/banner1', 'recommend/03/banner2'],
          goodId: '8810',
          goodImg: 'recommend/03/8810',
          buyPrice: '360',
          cash: 9999,
          detailImg: ['recommend/03/8810-1', 'recommend/03/8810-2', 'recommend/03/8810-3', 'recommend/03/8810-4', 'recommend/03/8810-5']
        }
      },
      {
        id: '04', prod: '89', imgFile: 'recommend/04', name: '泰瑟远程电击器（5米）', price: '450',
        detail: {
          banner: ['recommend/04/banner1', 'recommend/04/banner2'],
          goodId: 'yc',
          goodImg: 'recommend/04/yc',
          buyPrice: '675',
          cash: 9999,
          detailImg: ['recommend/04/yc-01', 'recommend/04/yc-02', 'recommend/04/yc-03', 'recommend/04/yc-04', 'recommend/04/yc-05.gif']
        }
      },
    ]
  },
  {
    type: 'carries',
    data: [
      {
        id: '01', prod: '82', imgFile: 'carries/01', name: 'X5强光钛合金高压电击棍', price: '280',
        detail: {
          banner: ['carries/01/banner1', 'carries/01/banner2'],
          goodId: 'X5',
          goodImg: 'carries/01/x5',
          buyPrice: '420',
          cash: 9999,
          detailImg: ['carries/01/x5-01', 'carries/01/x5-02', 'carries/01/x5-03', 'carries/01/x5-04', 'carries/01/x5-05', 'carries/01/x5-06']
        }
      },
      {
        id: '02', prod: '93', imgFile: 'carries/02', name: '特战反恐高压迷你电击棍', price: '280',
        detail: {
          banner: ['carries/02/banner1'],
          goodId: '6800',
          goodImg: 'carries/02/6800',
          buyPrice: '420',
          cash: 9999,
          detailImg: ['carries/02/6800-01', 'carries/02/6800-02', 'carries/02/6800-03', 'carries/02/6800-04', 'carries/02/6800-05', 'carries/02/6800-06']
        }
      },
      {
        id: '03', prod: '87', imgFile: 'carries/03', name: '1128掌中宝高压电击器', price: '220',
        detail: {
          banner: ['carries/03/banner1', 'carries/03/banner2'],
          goodId: '1128',
          goodImg: 'carries/03/1128',
          buyPrice: '264',
          cash: 9999,
          detailImg: ['carries/03/1128-01', 'carries/03/1128-02', 'carries/03/1128-03', 'carries/03/1128-04']
        }
      },
      {
        id: '04', prod: '80', imgFile: 'carries/04', name: '大飞鹰尖刺头高压电击器', price: '320',
        detail: {
          banner: ['carries/04/banner1', 'carries/04/banner2', 'carries/04/banner3', 'carries/04/banner4'],
          goodId: 'dfy',
          goodImg: 'carries/04/dfy',
          buyPrice: '384',
          cash: 9999,
          detailImg: ['carries/04/dfy-01', 'carries/04/dfy-02', 'carries/04/dfy-03', 'carries/04/dfy-04']
        }
      },
      {
        id: '05', prod: '79', imgFile: 'carries/05', name: '1101款小型电击棍', price: '160',
        detail: {
          banner: ['carries/05/banner1', 'carries/05/banner2', 'carries/05/banner3', 'carries/05/banner4'],
          goodId: '1101',
          goodImg: 'carries/05/1101',
          buyPrice: '192',
          cash: 9999,
          detailImg: ['carries/05/1101-01', 'carries/05/1101-02', 'carries/05/1101-03', 'carries/05/1101-04', 'carries/05/1101-05', 'carries/05/1101-06']
        }
      },
      {
        id: '06', prod: '76', imgFile: 'carries/06', name: 'X战警军工防滑高压电棍', price: '380',
        detail: {
          banner: ['carries/06/banner1', 'carries/06/banner2', 'carries/06/banner3', 'carries/06/banner4'],
          goodId: 'X4',
          goodImg: 'carries/06/X4',
          buyPrice: '570',
          cash: 9999,
          detailImg: ['carries/06/x4-01', 'carries/06/x4-02', 'carries/06/x4-03', 'carries/06/x4-04']
        }
      },
      {
        id: '07', prod: '82', imgFile: 'carries/07', name: 'X5强光钛合金高压电击棍', price: '280',
        detail: {
          banner: ['carries/01/banner1', 'carries/01/banner2'],
          goodId: 'X5',
          goodImg: 'carries/01/x5',
          buyPrice: '420',
          cash: 9999,
          detailImg: ['carries/01/x5-01', 'carries/01/x5-02', 'carries/01/x5-03', 'carries/01/x5-04', 'carries/01/x5-05', 'carries/01/x5-06']
        }
      },
      {
        id: '08', prod: '85', imgFile: 'carries/08', name: '928方形防滑高压电击器', price: '180',
        detail: {
          banner: ['carries/08/banner1', 'carries/08/banner2'],
          goodId: '928',
          goodImg: 'carries/08/928',
          buyPrice: '216',
          cash: 9999,
          detailImg: ['carries/08/928-01', 'carries/08/928-02', 'carries/08/928-03', 'carries/08/928-04', 'carries/08/928-05']
        }
      },
      {
        id: '09', prod: '68', imgFile: 'carries/09', name: '新款1315超高压航空合金电击棍', price: '340',
        detail: {
          banner: ['carries/09/banner1', 'carries/09/banner2'],
          goodId: '1315',
          goodImg: 'carries/09/1315',
          buyPrice: '510',
          cash: 9999,
          detailImg: ['carries/09/1315-01', 'carries/09/1315-02', 'carries/09/1315-03', 'carries/09/1315-04', 'carries/09/1315-05']
        }
      },
      {
        id: '10', prod: '77', imgFile: 'carries/10', name: '新款6610强光高压电击棍', price: '280',
        detail: {
          banner: ['carries/10/banner1'],
          goodId: '6610',
          goodImg: 'carries/10/6610',
          buyPrice: '336',
          cash: 9999,
          detailImg: ['carries/10/6610-01', 'carries/10/6610-02', 'carries/10/6610-03', 'carries/10/6610-04', 'carries/10/6610-05', 'carries/10/6610-06']
        }
      },
      {
        id: '11', prod: '78', imgFile: 'carries/11', name: 'T10强光高压电击棍', price: '240',
        detail: {
          banner: ['carries/11/banner1'],
          goodId: 'T10',
          goodImg: 'carries/11/t10',
          buyPrice: '288',
          cash: 9999,
          detailImg: ['carries/11/t10-01', 'carries/11/t10-02']
        }
      },
      {
        id: '12', prod: '91', imgFile: 'carries/12', name: '新款金刚狼牙007高压电棍', price: '300',
        detail: {
          banner: ['recommend/02/banner1', 'recommend/02/banner2', 'recommend/02/banner3', 'recommend/02/banner4', 'recommend/02/banner5'],
          goodId: '07',
          goodImg: 'recommend/02/007',
          buyPrice: '360',
          cash: 9999,
          detailImg: ['recommend/02/007-01', 'recommend/02/007-02', 'recommend/02/007-03']
        }
      },
      {
        id: '13', prod: '89', imgFile: 'carries/13', name: '泰瑟远程电击器（5米）', price: '450',
        detail: {
          banner: ['recommend/04/banner1', 'recommend/04/banner2'],
          goodId: 'yc',
          goodImg: 'recommend/04/yc',
          buyPrice: '675',
          cash: 9999,
          detailImg: ['recommend/04/yc-01', 'recommend/04/yc-02', 'recommend/04/yc-03', 'recommend/04/yc-04', 'recommend/04/yc-05.gif']
        }
      },
      {
        id: '14', prod: '90', imgFile: 'carries/14', name: '8810可调强光高压电击棍', price: '240',
        detail: {
          banner: ['recommend/03/banner1', 'recommend/03/banner2'],
          goodId: '8810',
          goodImg: 'recommend/03/8810',
          buyPrice: '360',
          cash: 9999,
          detailImg: ['recommend/03/8810-1', 'recommend/03/8810-2', 'recommend/03/8810-3', 'recommend/03/8810-4', 'recommend/03/8810-5']
        }
      },
    ]
  },
  {
    type: 'long',
    data: [
      {
        id: '01', prod: '70', imgFile: 'long/01', name: '新款X10自由伸缩高压电击棍', price: '340',
        detail: {
          banner: ['long/01/banner1', 'long/01/banner2', 'long/01/banner3'],
          goodId: 'X10',
          goodImg: 'long/01/x10',
          buyPrice: '510',
          cash: 9999,
          detailImg: ['long/01/x10-01', 'long/01/x10-02', 'long/01/x10-03', 'long/01/x10-04', 'long/01/x10-05', 'long/01/x10-06', 'long/01/x10-07']
        }
      },
      {
        id: '02', prod: '72', imgFile: 'long/02', name: '经典X8击打高压电击棍', price: '280',
        detail: {
          banner: ['long/02/banner1', 'long/02/banner2', 'long/02/banner3', 'long/02/banner4'],
          goodId: 'X8',
          goodImg: 'long/02/X8',
          buyPrice: '420',
          cash: 9999,
          detailImg: ['long/02/X8-01', 'long/02/X8-02', 'long/02/X8-03', 'long/02/X8-04', 'long/02/X8-05']
        }
      },
      {
        id: '03', prod: '74', imgFile: 'long/03', name: '1139无声电击高压电击棒', price: '280',
        detail: {
          banner: ['long/03/banner1', 'long/03/banner2', 'long/03/banner3'],
          goodId: '1139',
          goodImg: 'long/03/1139',
          buyPrice: '420',
          cash: 9999,
          detailImg: ['long/03/1139-01', 'long/03/1139-02', 'long/03/1139-03', 'long/03/1139-04', , 'long/03/1139-05']
        }
      },
      {
        id: '04', prod: '70', imgFile: 'long/04', name: '新款X10自由伸缩高压电击棍', price: '340',
        detail: {
          banner: ['long/01/banner1', 'long/01/banner2', 'long/01/banner3'],
          goodId: 'X10',
          goodImg: 'long/01/x10',
          buyPrice: '510',
          cash: 9999,
          detailImg: ['long/01/x10-01', 'long/01/x10-02', 'long/01/x10-03', 'long/01/x10-04', 'long/01/x10-05', 'long/01/x10-06', 'long/01/x10-07']
        }
      },
      {
        id: '05', prod: '75', imgFile: 'long/05', name: '939消音高压电击棒', price: '260',
        detail: {
          banner: ['long/05/banner1', 'long/05/banner2', 'long/05/banner3'],
          goodId: '939',
          goodImg: 'long/05/939',
          buyPrice: '312',
          cash: 9999,
          detailImg: ['long/05/939-01', 'long/05/939-02', 'long/05/939-03', 'long/05/939-04']
        }
      },
      {
        id: '06', prod: '71', imgFile: 'long/06', name: '1118B狼牙击打高压电棍', price: '300',
        detail: {
          banner: ['long/06/banner1', 'long/06/banner2', 'long/06/banner3'],
          goodId: '1118',
          goodImg: 'long/06/1118',
          buyPrice: '450',
          cash: 9999,
          detailImg: ['long/06/1118-01', 'long/06/1118-02', 'long/06/1118-03', 'long/06/1118-04', 'long/06/1118-05', 'long/06/1118-06']
        }
      },
      {
        id: '07', prod: '73', imgFile: 'long/07', name: '303风火轮超长高压电击棒', price: '460',
        detail: {
          banner: ['long/07/banner1'],
          goodId: '303',
          goodImg: 'long/07/303',
          buyPrice: '552',
          cash: 9999,
          detailImg: ['long/07/303-01', 'long/07/303-02', 'long/07/303-03', 'long/07/303-04']
        }
      },
    ]
  },
  {
    type: 'female',
    data: [
      {
        id: '01', prod: '67', imgFile: 'female/01', name: 'X6款高压电击器', price: '260',
        detail: {
          banner: ['female/01/banner1', 'female/01/banner2', 'female/01/banner3'],
          goodId: 'X6',
          goodImg: 'female/01/X6',
          buyPrice: '312',
          cash: 9999,
          detailImg: ['female/01/X6-01', 'female/01/X6-02', 'female/01/X6-03', 'female/01/X6-04']
        }
      },
      {
        id: '02', prod: '52', imgFile: 'female/02', name: '910迷你小钢炮高压电棍', price: '220',
        detail: {
          banner: ['female/02/banner1', 'female/02/banner2', 'female/02/banner3', 'female/02/banner4', 'female/02/banner5'],
          goodId: '910',
          goodImg: 'female/02/910',
          buyPrice: '330',
          cash: 9999,
          detailImg: ['female/02/910-01', 'female/02/910-02', 'female/02/910-03', 'female/02/910-04', 'female/02/910-05', 'female/02/910-06', 'female/02/910-07', 'female/02/910-08']
        }
      },
      {
        id: '03', prod: '66', imgFile: 'female/03', name: '女士口红防狼电击棍', price: '160',
        detail: {
          banner: ['female/03/banner1', 'female/03/banner2', 'female/03/banner3', 'female/03/banner4'],
          goodId: 'khdg',
          goodImg: 'female/03/khdg',
          buyPrice: '192',
          cash: 9999,
          detailImg: ['female/03/khdg-01', 'female/03/khdg-02', 'female/03/khdg-03']
        }
      },
      {
        id: '04', prod: '54', imgFile: 'female/04', name: 'A1移动电源高压电击器', price: '260',
        detail: {
          banner: ['female/04/banner1', 'female/04/banner2'],
          goodId: 'A1',
          goodImg: 'female/04/A1',
          buyPrice: '312',
          cash: 9999,
          detailImg: ['female/04/A1-01', 'female/04/A1-02', 'female/04/A1-03', 'female/04/A1-04', 'female/04/A1-05']
        }
      },
      {
        id: '05', prod: '60', imgFile: 'female/05', name: '磨砂口红款女士防狼喷雾', price: '80',
        detail: {
          banner: ['female/05/banner1'],
          goodId: 'mskh',
          goodImg: 'female/05/mskh',
          buyPrice: '96',
          cash: 9999,
          detailImg: ['female/05/mskh-01', 'female/05/mskh-02']
        }
      },
      {
        id: '06', prod: '67', imgFile: 'female/06', name: 'X6款高压电击器', price: '260',
        detail: {
          banner: ['female/01/banner1', 'female/01/banner2', 'female/01/banner3'],
          goodId: 'X6',
          goodImg: 'female/01/X6',
          buyPrice: '312',
          cash: 9999,
          detailImg: ['female/01/X6-01', 'female/01/X6-02', 'female/01/X6-03', 'female/01/X6-04']
        }
      },
      {
        id: '07', prod: '63', imgFile: 'female/07', name: 'RY防狼喷雾电影心花怒放女主同款', price: '108',
        detail: {
          banner: ['female/07/banner1'],
          goodId: 'RY',
          goodImg: 'female/07/RY',
          buyPrice: '162',
          cash: 9999,
          detailImg: ['female/07/RY-01', 'female/07/RY-02']
        }
      },
      {
        id: '08', prod: '96', imgFile: 'female/08', name: '女士口红款防狼喷雾', price: '68',
        detail: {
          banner: ['female/08/banner1'],
          goodId: 'khpw',
          goodImg: 'female/08/khpw',
          buyPrice: '102',
          cash: 9999,
          detailImg: ['female/08/khpw-01', 'female/08/khpw-02']
        }
      },
    ]
  },
  {
    type: 'atomization',
    data: [
      {
        id: '01', prod: '56', imgFile: 'atomization/01', name: '007红色闪电水柱喷射大容量防狼喷雾', price: '168',
        detail: {
          banner: ['atomization/01/banner1', 'atomization/01/banner2'],
          goodId: '007',
          goodImg: 'atomization/01/007',
          buyPrice: '252',
          cash: 9999,
          detailImg: ['atomization/01/007-01', 'atomization/01/007-02', 'atomization/01/007-03']
        }
      },
      {
        id: '02', prod: '94', imgFile: 'atomization/02', name: '警用超大容量防狼防暴喷雾', price: '108',
        detail: {
          banner: ['atomization/02/banner1'],
          goodId: ' jypw',
          goodImg: 'atomization/02/jypw',
          buyPrice: '162',
          cash: 9999,
          detailImg: ['atomization/02/jypw-01', 'atomization/02/jypw-02']
        }
      },
      {
        id: '03', prod: '64', imgFile: 'atomization/03', name: '大容量保安型防暴喷雾', price: '80',
        detail: {
          banner: ['atomization/03/banner1'],
          goodId: 'bax',
          goodImg: 'atomization/03/bax',
          buyPrice: '96',
          cash: 9999,
          detailImg: ['atomization/03/bax-01', 'atomization/03/bax-02']
        }
      },
      {
        id: '04', prod: '61', imgFile: 'atomization/04', name: '原装警用水柱型大容量防狼喷雾', price: '138',
        detail: {
          banner: ['atomization/04/banner1'],
          goodId: '511',
          goodImg: 'atomization/04/511',
          buyPrice: '207',
          cash: 9999,
          detailImg: ['atomization/04/511-01', 'atomization/04/511-02', 'atomization/04/511-03']
        }
      },
      {
        id: '05', prod: '59', imgFile: 'atomization/05', name: 'NATO进口款大容量防狼喷雾', price: '120',
        detail: {
          banner: ['atomization/05/banner1'],
          goodId: 'NATO',
          goodImg: 'atomization/05/NATO',
          buyPrice: '144',
          cash: 9999,
          detailImg: ['atomization/05/NATO-01', 'atomization/05/NATO-02']
        }
      },
      {
        id: '06', prod: '58', imgFile: 'atomization/06', name: '美国进口CSA大容量防狼喷雾', price: '150',
        detail: {
          banner: ['atomization/06/banner1'],
          goodId: 'CSAA',
          goodImg: 'atomization/06/CSAA',
          buyPrice: '180',
          cash: 9999,
          detailImg: ['atomization/06/CSAA-01', 'atomization/06/CSAA-02']
        }
      },
      {
        id: '07', prod: '69', imgFile: 'atomization/07', name: '进口迷你body防狼喷雾', price: '118',
        detail: {
          banner: ['atomization/07/banner1'],
          goodId: 'bod',
          goodImg: 'atomization/07/bod',
          buyPrice: '177',
          cash: 9999,
          detailImg: ['atomization/07/bod-01', 'atomization/07/bod-02']
        }
      },


      {
        id: '08', prod: '60', imgFile: 'atomization/08', name: '磨砂口红款女士防狼喷雾', price: '80',
        detail: {
          banner: ['female/05/banner1'],
          goodId: 'mskh',
          goodImg: 'female/05/mskh',
          buyPrice: '96',
          cash: 9999,
          detailImg: ['female/05/mskh-01', 'female/05/mskh-02']
        }
      },
      {
        id: '09', prod: '63', imgFile: 'atomization/09', name: 'RY防狼喷雾电影心花怒放女主同款', price: '108',
        detail: {
          banner: ['female/07/banner1'],
          goodId: 'RY',
          goodImg: 'female/07/RY',
          buyPrice: '162',
          cash: 9999,
          detailImg: ['female/07/RY-01', 'female/07/RY-02']
        }
      },
      {
        id: '10', prod: '65', imgFile: 'atomization/10', name: '老美警用防暴防狼喷雾', price: '108',
        detail: {
          banner: ['atomization/10/banner1'],
          goodId: 'lm',
          goodImg: 'atomization/10/lm',
          buyPrice: '162',
          cash: 9999,
          detailImg: ['atomization/10/lm-01', 'atomization/10/lm-02']
        }
      },
      {
        id: '11', prod: '96', imgFile: 'atomization/11', name: '女士口红款防狼喷雾', price: '68',
        detail: {
          banner: ['female/08/banner1'],
          goodId: 'khpw',
          goodImg: 'female/08/khpw',
          buyPrice: '102',
          cash: 9999,
          detailImg: ['female/08/khpw-01', 'female/08/khpw-02']
        }
      },
    ]
  }
]
